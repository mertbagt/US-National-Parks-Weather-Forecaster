...
function formatMaxResults(element) {
  let count = element + 1;
  if (count == 10) {
    $("#js-max-results").append(
      $("<option selected></option>").val(count).html(count)
    );
  } else {
...
function displayResults(responseJson) {
  ...
  for (let i = 0; i < ParkForecaster.resultsArray.length; i++) {
    ParkForecaster.resultsArray[i].addresses.forEach((element) =>
      formatAddress(element, i)
    );
    $("#results-list").append(`
      <li class="results-li">
        <a href="${ParkForecaster.resultsArray[i].url}" target="_blank" rel="noreferrer">${ParkForecaster.resultsArray[i].fullName}</a>
    ...
    .catch((err) => {
      ...
      if (err.message == "") {
        ...
        $("#js-error-message").append(
          `Something went wrong with ${endpoint}: ${err.message}.  `
        );
      }
...
function getNWSGridPoints() {
  ...
  const url =
    ParkForecaster.nwsURL + ParkForecaster.lat + "%2C" + ParkForecaster.long;
...
function watchResults() {
  $("#results-list").on("click", ".btn-click-action", function (event) {
    ...
    ParkForecaster.chosenParkName =
      ParkForecaster.resultsArray[chosenPark].fullName;
